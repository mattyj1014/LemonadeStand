...
//  LemonadeStand
//
//  Created by Matthew Jenks on 9/19/14.
...
class ViewController: UIViewController {
    ...
    @IBAction func startDatButtonPressed(sender: AnyObject)
    {
        ...
        let lemondadeRatio = Float(lemonsToMix) / Float(iceCubesToMix)
        println("lemonade ratio: \(lemondadeRatio)")
        
        for x in 0...customers {
           
            let preference = Double(arc4random_uniform(UInt32(100))) / 100
            
            if preference < 0.4 && lemondadeRatio > 1 {
                supplies.money += 1
                println("Paid")
            }
            else if preference > 0.6 && lemondadeRatio < 1 {
                supplies.money += 1
                println("Paid")
            }
            else if preference <= 0.6 && preference >= 0.4 && lemondadeRatio == 1 {
                supplies.money += 1
                println("Paid")
            }
            else {
